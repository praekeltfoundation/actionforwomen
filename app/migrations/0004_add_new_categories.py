...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        if not Category.objects.filter(slug='celebrity').exists():
            Category.objects.create(title='Celebrity', slug='celebrity')
        if not Category.objects.filter(slug='fact').exists():
            Category.objects.create(title='Fact', slug='fact')

    def backwards(self, orm):
        Category = orm['category.Category']
        if Category.objects.filter(slug='celebrity').exists():
            Category.objects.get(slug='celebrity').delete()
        if Category.objects.filter(slug='fact').exists():
            Category.objects.get(slug='fact').delete()
