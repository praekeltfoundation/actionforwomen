...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        if not Category.objects.filter(slug='stories').exists():
            Category.objects.create(title='Stories', slug='stories')
        if not Category.objects.filter(slug='faq').exists():
            Category.objects.create(title='FAQs', slug='faq')
        if not Category.objects.filter(slug='support').exists():
            Category.objects.create(title='Support', slug='support')

    def backwards(self, orm):
        Category = orm['category.Category']
        if Category.objects.filter(slug='stories').exists():
            Category.objects.get(slug='stories').delete()
        if Category.objects.filter(slug='faq').exists():
            Category.objects.get(slug='faq').delete()
        if Category.objects.filter(slug='support').exists():
            Category.objects.get(slug='support').delete()
