...
class Command(BaseCommand):
    ...
    def push(self, datetime_obj):
        ...
        range_start_cumulative = datetime(year=2012, month=1, day=1)

        client = Client(server='http://holodeck.praekelt.com')
        ga_service = utils.get_service()

        mobi_comments = Comment.objects.filter(
            Q(user__userprofile__origin='mobi') |
        ...
        client.send(
            samples=(
                ("Visitors", results['totalsForAllResults']['ga:visitors']),
                ("Registration", mobi_users.filter(
        ...
        client.send(
            samples=(
                ("Prenatal", mobi_userprofile.filter(
                    delivery_date__gte=datetime_obj).count()),
                ("Postnatal", mobi_userprofile.filter(
