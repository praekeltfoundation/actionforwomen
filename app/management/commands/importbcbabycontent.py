...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        baby_category, created = Category.objects.get_or_create(title="My Baby", slug="my-baby")
        ...
        for node in pages.getElementsByTagName('page'):
            ...
            if len(categories) == 1:
                ...
                primary_category = baby_category

            for link in node.getElementsByTagName('link'):
                if pk not in links:
                    links[pk] = []
                links[pk].append((8000 + int(link.attributes['uid'].value), link.firstChild.data))
